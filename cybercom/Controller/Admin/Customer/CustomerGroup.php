...
class CustomerGroup extends \Controller\Core\Admin{
	...
	public function saveAction() {
			...
			try{
				...
				if ($id = (int)$this->getRequest()->getGet('groupId')) {
					...
					$this->getAdminMessage()->setSuccess("Record Updated Successfully");
				
				} else {

					$customerGroup->createdAt = date('Y-m-d H:i:s');
					$this->getAdminMessage()->setSuccess("Record Inserted Successfully");
				}
	...
	public function deleteAction() {
		...
		try{
			...
			if (!$id) {
				$this->getAdminMessage()->setFailure("Id Not Found");
			}
			$customerGroup = $this->getCustomerGroupModel();
			$customerGroup->load($id);
			if(!$customerGroup->delete())
			{
				$this->getAdminMessage()->setFailure("Id Invalid");
			}
			...
			$response = [
				'status' => 'success',
				'message' => 'customerGroup displayed',
				'element' => [
					[
						'selector' => '#moduleGrid',
						'html' => $grid
