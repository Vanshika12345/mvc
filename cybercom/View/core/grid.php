							 				...
							 				<?php foreach ($actions as $key => $action): ?>
							 					<?php if($action['ajax']):?>
							 						<?php if($action['label'] == 'Edit'):?>
							 							<button type="button" class="btn btn-warning" onclick="<?= $this->getMethodUrl($row,$action['method']);?>"><?= $action['label']; ?></button>&nbsp;
...
<!-- <li class="page-item">
			      <a class="page-link" href="javascript:void(0);" tabindex="-1" onclick="object.setUrl('<?php //$this->getUrl()->getUrl('grid',null,['page'=> $pages->getPrevious()],true);?>').resetParam().load();">Previous</a> -->
				<!-- <li class="page-item">
			      <a class="page-link" href="javascript:void(0);" onclick="object.setUrl('<?php //$this->getUrl()->getUrl('grid',null,['page'=> $pages->getNext()],true);?>').resetParam().load();">Next</a>
			    </li> -->